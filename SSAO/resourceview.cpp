...
namespace framework
{
	...
	void ResourceView::init(ID3D11Device* device, ID3D11Resource* resource, unsigned int bindFlags)
	{
		destroy();
		...
		if ((bindFlags & D3D11_BIND_SHADER_RESOURCE) == D3D11_BIND_SHADER_RESOURCE)
		{
			...
			HRESULT hr = device->CreateShaderResourceView(resource, desc, &m_shaderView);
			if (hr != S_OK)
			{
		...
		if ((bindFlags & D3D11_BIND_RENDER_TARGET) == D3D11_BIND_RENDER_TARGET)
		{
			...
			HRESULT hr = device->CreateRenderTargetView(resource, desc, &m_renderTargetView);
			if (hr != S_OK)
			{
		...
		if ((bindFlags & D3D11_BIND_DEPTH_STENCIL) == D3D11_BIND_DEPTH_STENCIL)
		{
			...
			HRESULT hr = device->CreateDepthStencilView(resource, desc, &m_depthStencilView);
			if (hr != S_OK)
			{
		...
		if ((bindFlags & D3D11_BIND_UNORDERED_ACCESS) == D3D11_BIND_UNORDERED_ACCESS)
		{
			...
			HRESULT hr = device->CreateUnorderedAccessView(resource, desc, &m_uaView);
			if (hr != S_OK)
			{
