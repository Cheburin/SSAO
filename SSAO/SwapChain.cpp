	...
	const DXGI_SURFACE_DESC* backBufferSurfaceDesc, void* pUserContext)
{
	...
	float np = 0.01, fp = 100, fov = D3DX_PI / 3;
	...
	SCG->RenderTargetT.resize(2);
	SCG->RenderTargetV.resize(2);
	SCG->RenderTargetSRV.resize(2);

	for (int i = 0; i < 2; i++){

		hr = device->CreateTexture2D(&textureDesc, nullptr, SCG->RenderTargetT[i].ReleaseAndGetAddressOf());

		hr = device->CreateRenderTargetView(SCG->RenderTargetT[i].Get(), nullptr, SCG->RenderTargetV[i].ReleaseAndGetAddressOf());

		hr = device->CreateShaderResourceView(SCG->RenderTargetT[i].Get(), nullptr, SCG->RenderTargetSRV[i].ReleaseAndGetAddressOf());
	}
