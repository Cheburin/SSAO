...
float4 LAMBERT(in float3 pos : TEXCOORD0, in float3 normal : TEXCOORD1, in float2 tex_0 : TEXCOORD2, in float4 tex_1 : TEXCOORD3):SV_TARGET
{ 
   float4 clip_pos = tex_1 / tex_1.w;
   ...
   float3 diffColor = float3(-1>0?1:0,-1<0?1:0,0);
   float3 specColor = float3(1,1,1);

   float   Kd = max         ( 0.09,  abs(dot ( l, n ) ) );
   float   Ks = pow         ( max ( 0.0, dot ( h, n ) ), 40.0 );
    
   return float4( Kd*diffColor + Ks*specColor, 0.5 );    
}